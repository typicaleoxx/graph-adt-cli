...
// handle inserting new vertex into the graph with validation
...
// handle removing specific edge between two vertices with validation
...
// handle displaying all vertices in the graph with error check
void handleDisplayVertices(Graph* graph) {
    vector<Vertex*> vertices = graph->vertices();
    
    if (vertices.empty()) {
        cout << "error: no vertices in graph" << endl;
        return;
    }
...
// handle checking adjacency between two vertices with validation
void handleCheckAdjacency(Graph* graph) {
    ...
    // validate vertices are different
    if (v1 == v2) {
        cout << "error: cannot compare vertex with itself" << endl;
        return;
    }
